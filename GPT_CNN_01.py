...
# 1～5層のデータを作成
layer_1 = np.random.rand(num_samples, height, width)  # 標高値
layer_2 = np.random.rand(num_samples, height, width)  # 傾斜量
layer_3 = np.random.rand(num_samples, height, width)  # 降水量
layer_4 = np.random.rand(num_samples, height, width)  # 植生分布
layer_5 = np.random.rand(num_samples, height, width)  # 地質情報

# 6層目のデータを作成（崩壊分布、2%の確率で崩壊）
layer_6 = np.random.choice([0, 1], size=(num_samples, height, width), p=[0.98, 0.02])
...
# データのリシェイプ（正しい形状に変換）
input_data = input_data.reshape(num_samples, height, width, 5)
target_data = target_data.reshape(num_samples, height, width, 1)

# データのリシェイプが正しいことを確認
print(input_data.shape)  # (1000, 300, 300, 5)
print(target_data.shape)  # (1000, 300, 300, 1)
...
non_collapse_indices_sampled = np.random.choice(np.arange(len(non_collapse_indices[0])), size=num_collapse*5, replace=False)
...
sampled_indices = (np.concatenate((collapse_indices[0], non_collapse_indices[0][non_collapse_indices_sampled])),
                   ...
                   np.concatenate((collapse_indices[3], non_collapse_indices[3][non_collapse_indices_sampled])))
